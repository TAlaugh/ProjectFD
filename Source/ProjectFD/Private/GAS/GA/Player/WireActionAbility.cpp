...
UWireActionAbility::UWireActionAbility()
{
...
bool UWireActionAbility::CanActivateAbility(const FGameplayAbilitySpecHandle Handle,
...
void UWireActionAbility::ActivateAbility(const FGameplayAbilitySpecHandle Handle,
	...
	const FGameplayEventData* TriggerEventData)
{
	...
	if (HasAuthorityOrPredictionKey(ActorInfo, &ActivationInfo))
	{
		...
		PlaySkillMontageTask->OnCompleted.AddDynamic(this, &UWireActionAbility::LoopMontagePlay);
		PlaySkillMontageTask->OnBlendOut.AddDynamic(this, &UWireActionAbility::LoopMontagePlay);
		PlaySkillMontageTask->OnInterrupted.AddDynamic(this, &UWireActionAbility::K2_EndAbility);
		PlaySkillMontageTask->OnCancelled.AddDynamic(this, &UWireActionAbility::K2_EndAbility);
		...
		WaitSkillEventTask->EventReceived.AddDynamic(this, &UWireActionAbility::LaunchWire);
		...
		// WaitRelease->OnRelease.AddDynamic(this, &UWireActionAbility::OnInputReleased);
		// WaitRelease->ReadyForActivation();
	}
}

void UWireActionAbility::CancelAbility(const FGameplayAbilitySpecHandle Handle,
	const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo,
...
void UWireActionAbility::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo,
...
void UWireActionAbility::LaunchWire(FGameplayEventData EventData)
{
	AFDPlayerCharacter* PC = Cast<AFDPlayerCharacter>(GetAvatarActorFromActorInfo());
	...
	if (World)
	{
		World->GetTimerManager().SetTimer(EndTimerHandle, this, &UWireActionAbility::K2_EndAbility, SkillDuration, false);
	}
	...
	BeginFlying();
	bMoving = true;
	World->GetTimerManager().SetTimer(MoveTimerHandle, this, &UWireActionAbility::TickWireMove, TickInterval, true);
}

void UWireActionAbility::OnInputReleased(float TimeHeld)
{
	CancelAbility(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, true);
}

void UWireActionAbility::TickWireMove()
{
	AFDPlayerCharacter* PC = Cast<AFDPlayerCharacter>(GetAvatarActorFromActorInfo());
...
void UWireActionAbility::SpawnCable(const FVector& StartWorldLoc, const FName& StartSocketName,
...
void UWireActionAbility::DestroyCable()
{
	if (CableActor.IsValid())
	{
		if (UCableComponent* CC = CableActor->CableComponent)
...
void UWireActionAbility::BeginFlying()
{
	ACharacter* Character = Cast<ACharacter>(CurrentActorInfo ? CurrentActorInfo->AvatarActor.Get() : nullptr);
	if (!Character) return;
...
void UWireActionAbility::EndFlying(bool bKeepMomentum)
{
	// 틱 정지 및 케이블 제거
	if (UWorld* World = GetWorld())
	{
...
void UWireActionAbility::LoopMontagePlay()
{
	...
	PlaySkillMontageTask->OnCompleted.AddDynamic(this, &UWireActionAbility::K2_EndAbility);
	PlaySkillMontageTask->OnBlendOut.AddDynamic(this, &UWireActionAbility::K2_EndAbility);
	PlaySkillMontageTask->OnInterrupted.AddDynamic(this, &UWireActionAbility::K2_EndAbility);
	PlaySkillMontageTask->OnCancelled.AddDynamic(this, &UWireActionAbility::K2_EndAbility);
	PlaySkillMontageTask->ReadyForActivation();
}
